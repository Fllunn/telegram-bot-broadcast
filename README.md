...
- **Списки групп**: загрузка `.xlsx`/`.xls`, нормализация ссылок/юзернеймов, дедупликация по chat id/username/link, статистика.
...
### Загрузка файлов групп
- Форматы: `.xlsx`, `.xls` (используется первый лист)
- Колонки (регистр не важен): `Название`, `Username`, `Ссылка` (лишние игнорируются; заголовок распознаётся)
- Ссылки нормализуются к `https://t.me/...`, `@` у username удаляется, дубли удаляются по chat id/username/link
- Записи с `is_member=false` (нет доступа) пропускаются
- Статистика сохраняется в `metadata.broadcast_groups_stats` и используется для оценки времени
